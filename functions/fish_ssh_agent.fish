function sshagent_findsockets
    find $HOME/.ssh/agent/s.* -uid (id -u) -type s -name \*.agent.\* 2>/dev/null
end
...
function fish_ssh_agent
    ...
    # If there is no agent in the environment, search $HOME/.ssh/agent/s.*for
    ...
    if [ $status = 1 ]
        ...
        for key in id_rsa id_dsa id_ecdsa id_ed25519 identity
            test -f $HOME/.ssh/$key &&
